...
function getCreditsInformation (projectImages, projectFonts, projectOthers) {
  ...
  if ((projectImages[0].toLowerCase() === "skip") && (projectFonts[0].toLowerCase() === "skip"))
    ...
    for (let index = 0; i < sizeCredits; index++) {
      if ((projectImages.length > i) && (projectImages[i].toLowerCase() !== "skip")) {
        let credImages = creditsLinks.filter(element => element.type.toLowerCase() === projectImages[i].toLowerCase());

        imagesContent += `
        ['${credImages[0].type}'](${credImages[0].url})`;
      }
      if ((projectFonts.length > i) && (projectFonts[i].toLowerCase() !== "skip")) {
